 ...
 ******************************************************************************/ 
 dojo.provide("widgets.TemplateProperties");
...
dojo.declare("widgets.TemplateProperties",
             [dijit._Widget, dijit._Templated],
{
    widgetsInTemplate: true,
    templatePath: dojo.moduleUrl("widgets","template_properties.html"),
	...
	postCreate : function() {
		...
		this.tblColumns.push(this.gridhelper.setColumn("Name", "String", false, true, null, "setPropertyName", "getPropertyName", this.myObject, null ));
		...
		this._propertiesTable = new widgets.GenGrid({ grid_inputcolumns:  this.tblColumns, tblName: this.properiesTableName, hasDel: false, metaObject: this.myObject, 
		                                          	  metaAttrSize: "getNumberOfProperties", delAttrMethod: "deleteProperty", addButton: false, refreshButton:false });
  	 ...
  	 startup: function(){
        widgets.TemplateProperties.superclass.startup.apply(this, arguments);
 		
 		
	 	var optionsType = new dojo.data.ItemFileWriteStore({data: {identifier: 'name', items:[]}});
	 	for ( var p in TemplateEnum.TemplateType) {
	 		optionsType.newItem({name: TemplateEnum.TemplateType[p]});
		}
 		...
 		this.templateProp.expandCollapse();
